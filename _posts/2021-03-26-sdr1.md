...
excerpt: Introduction of Sufficient Dimension Reduction and Sliced Inverse Regression
---
...
Sufficient Dimension Reduction is kind of dimensionality reduction technique combining concepts of sufficient statistic and dimension reduction. 

This concepts are Unlike general dimension reduction technique which known as unsupervised learning like Principle component analysis or Cluster analysis, SDR is supervised learning. It aims to find smallest portion of independent variables for explaining dependent variable. It has theoretical structure as follow.
...
​                  $=\Sigma P_{\beta}(\Sigma)\Sigma^{-1} E(X \mid Y)$                        $\{P_{\beta}^t (\Sigma) = \Sigma P_{\beta}(\Sigma) \Sigma^{-1}\}$
...
However, even though SIR  are good for estimating SDR subspace, it has serious problem. That is if between the connection of Y and X, X has symmetric at 0, then it means $E(X\mid Y)=0$ ,so it did not deliver any information about SDR subspace. Therefore, other methods like SAVE, CR, DR are developed to overcome such limitation.
