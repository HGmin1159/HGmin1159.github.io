...
$\frac{1}{2^m}\overset{2^m-1}{\underset{k=0}{\sum}} e^{-\frac{2 \pi k i}{2^m} (j- 2^m\psi) }$

이는 각 큐빗의 평균값이므로  $E(e^{-\frac{2 \pi k i}{2^m} (j- 2^m\psi) })$의 추정치이며 이는 j의 확률진폭의 추정치라고 말할 수 있다. 
...
이는 정규분포와 매우 흡사하며 $j=2^m \psi$에서 가장 높은 확률값을 가지게 될 것이다. $\psi$는 0에서 1사이의 수이므로  결과물 j에 대해 $\frac{j}{2^m}=\hat{\psi}$를 추정할 수 있다. 
...
$0.b_1b_2b_3b_4...b_m$ $(b_i \in \{0,1\} \quad \forall i)$ 혹은 $b_i$는 $\ket {q}_{2^m}$의 i번째 큐빗의 관측값

 물론 아이젠밸류는 보통 무한대인 반면 QPE 알고리즘은 사용된 큐빗개수 m에 따라 10^{-n} 자리수 까지만 추정을 할 수 있기 때문에 정확한 추정을 하는 것은 거의 불가능하다. 
 
 따라서 오차율이 있는데 이는 반복측정을 통해서 낮출 수 있다. 또 m을 늘림에 따라서 근사 지점까지 추정을 할 수 있다. 


이를 활용한 알고리즘으로 HHL Algorithm For Linear Solver를 들 수 있다. Linear Solver라 함은 말 그대로 특정 행렬 A와 벡터 b에 대해  $Ax=b$를 만족하는 x를 찾는 것이다. 

 이 꼴 자체가 바로 사용될 수 도 있겠지만 이를 통해서 Quadratic Form $x^tMx$를 구해내거나, $A^{-1}$를 구하는 등에 응용할 수 있다. 구체적인 내용에 대해서는 차후에 다시 논문을 찾아보고 공부를 하도록 하자. 
