...
물론 아이젠밸류는 보통 무한대인 반면 QPE 알고리즘은 사용된 큐빗개수 m에 따라 10^{-n} 자리수 까지만 추정을 할 수 있기 때문에 정확한 추정을 하는 것은 거의 불가능하다. 따라서 오차율이 있는데 이는 반복측정을 통해서 낮출 수 있다. 또 m을 늘림에 따라서 근사 지점까지 추정을 할 수 있다. 
