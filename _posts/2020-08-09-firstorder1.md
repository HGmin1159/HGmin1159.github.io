---
title: \[수학\] First-Order Method part1 - Gradient Descent
categories: [Convex]
...
$\sum _{i=0}^{k} \{f(x^{i})-f(x^\ast)\} \leq \sum_{i=0}^k\{ \frac{1}{2t}(\parallel x^{i} -x^\ast\parallel_2^2 - \parallel x^{i+1}-x^\ast \parallel_2^2)\} \\\ \qquad \qquad \qquad \quad  \qquad = \frac{1}{2t} (\parallel x^0-x^\ast \parallel_2^2-\parallel x^{k+1}-x^\ast \parallel_2^2) \\\  \qquad \qquad \qquad \quad  \qquad \leq \frac{1}{2t}\parallel x^0 - x^\ast \parallel_2^2$
